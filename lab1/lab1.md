...
A: It is using recursion to find the fiboncci number (it calculates the two fibonacci numbers before that number and sums them to find out number)
2. What happens if we remove the "if ... return ..." and only keep the last line?
A: It gives the error: RecursionError: maximum recursion depth exceeded because the recursion never stops
3. What is fibonacci(20)? how much time did it take to calculate that?
A: 6765, calculating this took 1.9358e-03 seconds
4. What is fibonacci(30)? how much time did it take to calculate that?
A: 832040, calculating this took 1.3582e-01 seconds
5. How much time did it take you to calculate fibonacci(40)? (this might take a while...)
A: 102334155, calculating this took 1.7913e+01 seconds
```
...
2. How many function calls does fibonacci(5) take? 9
3. How many function calls does fibonacci(10) take? 109
4. Why is it so slow? Where does the complexity come from? Because it is repeatedly calculating the same work it computed before.
5. Is this O(n)? is this O(2^n)? Why? It is O(2^n) because it exponentially increases
6. Is this Ω(n)? Why? Yes (Big Omega represents the best case).
```
...
```
TODO:
1. How is this one different from the previous one? It has a list (cache) of numbers it calculated previously so it doesn't have to recalculate them.
2. How much time does it take to calculate fibonacci(30)? calculating this took 3.6600e-05 seconds.
3. Why is it often faster? it has less recursions; does less work
4. Also modify this file to count: how many times the function had to be called for fibonacci(30)? 59 times
5. Is this O(n)? is this O(2^n)? Why? it is O(n) because each number is calculated (and stored) once. It is also O(2^n) because Big O represents upper bound, so it can be anything higher than linear in this case.
6. Is this Ω(n)? is this Ω(2^n)? Why? It is Omega(n) because it is always linear (including best case)
```
